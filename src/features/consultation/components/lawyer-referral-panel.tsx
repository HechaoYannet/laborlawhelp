  ...
  return (
    <Card className="relative overflow-hidden rounded-[26px] border border-violet-200/80 bg-white/96 py-0 shadow-[0_18px_48px_rgba(15,23,42,0.08)] ring-1 ring-black/5">
      ...
      <CardContent className="space-y-4 px-5 pb-5 md:px-6 md:pb-6">
        <section className="rounded-[24px] border border-violet-200/80 bg-[linear-gradient(135deg,rgba(245,243,255,0.96),rgba(255,255,255,0.98))] p-4 shadow-sm">
          <div className="flex items-center gap-2 text-sm font-semibold text-slate-900">
            <ShieldAlert className="h-4 w-4 text-violet-600" />
            转介建议
          </div>
          <p className="mt-3 rounded-2xl border border-white/80 bg-white/90 px-4 py-3 text-sm leading-6 text-slate-700">
            {actionHint}
          </p>
          {riskTags.length > 0 ? (
          ...
          <section className="rounded-2xl border border-slate-200/80 bg-slate-50/80 p-4">
            ...
            <div className="mt-3 grid gap-3">
              {lawyers.map((lawyer, index) => (
                <div key={`${index}-${String(lawyer.id ?? '')}`} className="rounded-[22px] border border-slate-200/80 bg-white px-4 py-4 shadow-sm">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
